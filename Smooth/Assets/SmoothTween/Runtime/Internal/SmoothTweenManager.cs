    ...
    internal class SmoothTweenManager : MonoBehaviour
    {
        ...
        Tween addTween_internal([NotNull] ReusableTween tween)
        {
            ...
            if (warnTweenOnDisabledTarget)
            {
                ...
                {
                    Debug.LogWarning($"Tween is started on GameObject that is not active in hierarchy: {comp.name}. {Constants.buildWarningCanBeDisabledMessage(nameof(warnTweenOnDisabledTarget))}",
                        comp);
                }
            }
